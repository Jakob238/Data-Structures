...
void MTree<DT>::insert(const DT& value){
    // If the value already exists in the tree, throw exception
    if (search(value)) {
    ...
    if (is_leaf()) {
        // If there's space, add the value
        if (values.size() < M - 1) {
        ...
        // Case 2: If the node exceeds its capacity, split it
        else {
            split_node();
    ...
    // Case 3: If it's not a leaf, recurse on the appropriate child node
    else {
        find_child(value)->insert(value);
    }
}
